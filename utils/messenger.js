...
async function sendMessage(token, recipientId, text) {
  ...
  for (const chunk of chunks) {
    ...
    if (data.error) console.error('sendMessage error:', JSON.stringify(data.error));
  }
}

async function sendImageMessage(token, recipientId, imageUrl) {
  console.log('Sending image:', imageUrl.slice(0, 80));
  const res = await fetch(`${FB_API}/me/messages?access_token=${token}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  const data = await res.json();
  if (data.error) console.error('sendImage error:', JSON.stringify(data.error));
  else console.log('Image sent ok');
}
